...
if (profession == "Woodcutting"):
	...
	if (level == 70):
		...
		Node([475.5, 96.5, -548.5], tickskipping = True, delay = 0.0)
...
def imageProcessor(picture, health = False, doubleSpeed = 0, dura = False, eHealth = False):
	...
	if (health):
		...
		convertToWhite = (red != 0) | (green < 0.666) | (green > 0.668) | (blue < 0.666) | (blue > 0.668)
		picture[convertToWhite] = 1
		...
		convertToWhite = (red < 0.666) | (red > 0.667) | (green != 0) | (blue != 0)
		picture[convertToWhite] = 1
		convertToBlack = (red != 1) & (green != 1) & (blue != 1)
		picture[convertToBlack] = 0
	elif (eHealth):
...
def checkStream():
	screenshot = py.screenshot(region=(780, 30, 470, 70))
...
def checkDurability():
	screenshot = py.screenshot(region=(145, 915, 20, 20))
...
def enemyHealth():
	screenshot = py.screenshot(region=(775, 105, 370, 25))
...
def farm(previousHealth, durability, speedBomb, speedClock, left = True, delay = 7.0, autoClicker = False, keyPress = [False, False, False, False]):
	py.press('2')
	if (speedBomb[0]):
		delay /= 2.0
	num = random.random()
...
def mountHorse(keyPress):
	...
	while (True):
		...
		if (((checkHeight > 0.84) and (checkHeight <= 0.85)) or ((checkHeight > 0.34) and (checkHeight <= 0.35))):
			break
...
def fixTool(checkpoint, keyPress):
	...
	while (True):
		py.rightClick()
		location = py.locateOnScreen(directory + 'scrap.png', region=(940, 430, 40, 35))
		try:
			location.left
			futureTime = time.time() + 30
			while (True):
				...
				py.click()
				location = py.locateOnScreen(directory + 'repair.png', region=(1020, 330, 85, 25))
				try:
					location.left
					futureTime = time.time() + 30
					while (True):
						...
						location = py.locateOnScreen(directory + 'blank.png', region=(795, 355, 40, 40))
						try:
							location.left
							break
						except:
	...
	while (True):
		py.press('e')
		location = py.locateOnScreen(directory + 'questBook.png', region=(1060, 990, 40, 35))
		try:
			location.left
			break
		except:
...
def depositInBank(checkpoint, keyPress):
	...
	while (True):
		py.rightClick()
		location = py.locateOnScreen(directory + 'arrow.png', region=(1085, 340, 35, 35))
		try:
			location.left
			futureTime = time.time() + 30
			while (True):
				...
				py.click()
				location = py.locateOnScreen(directory + 'blank.png', region=(980, 585, 40, 40))
				try:
					location.left
					break
				except:
...
def repair(keyPress, deposit = True):
	stopMove(keyPress)
	previousPosition = [0.0, 0.0, 0.0]
	...
	for i in range(len(gps)):
		...
		if (checkpoint[1] == -1):
			...
			leap(checkpoint, keyPress = keyPress)
		elif (checkpoint[1] == -3):
			changeRotation(targetPitch = checkpoint[0], targetYaw = checkpoint[2], keyPress = keyPress)
...
def main():
	...
	try:
		...
		if (waterLevel != 0.0):
			speed = swimSpeed
		if (proximity([978.0, 73.0, -671.0], radius = 999, abortRadius = 499999, keyPress = keyPress)):
			repair(keyPress = keyPress, deposit = False)
		while (True):
			...
			for i in range(len(pointOfInterest)):
				...
				if (pointOfInterest[i].farmable):
					...
					if (pointOfInterest[i].tickskipping):
						previousHealth = farm(previousHealth, durability, speedBomb, speedClock, leftClick, farmTime - 2, pointOfInterest[i].rapidClick, keyPress = keyPress)
						if (previousHealth[1] == 1):
							break
						...
						previousHealth = farm(previousHealth, durability, speedBomb, speedClock, leftClick, farmTime, pointOfInterest[i].rapidClick, keyPress = keyPress)
						if (previousHealth[1] == 1):
							break
						if (previousHealth[1] + 1 == drawBlood[1]):
							tryAttack = True
...
main()
#calculateTheta([978.011, 73.0, -667.975], [982.108, 73.0, -667.975], True)
#getVisual(node = True, yLevel = 1.85)
#getPosition(display = True)
#getDirection(True)
#getVisual(True)
#getHealth(display = True)
#calculateSpeed(movementStatus = 1, display = True)
#repair([False, False, False, False], deposit = False)
#checkSpeed()
#print(enemyHealth())
#print(checkStream())
"""
