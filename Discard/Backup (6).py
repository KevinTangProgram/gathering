...
def stopMove(movementStatus = 0, walkingStick = False, shield = False):
	...
	if (movementStatus == 2):
		...
		py.keyUp('ctrlleft')
	if (walkingStick or shield or character == "Assassin"):
		py.press('2')
...
def getPosition(display=False, movementStatus = 0, iterations = 0, swim = [False]):
	try:
		...
		if (swim[0] == False and positionCoordinates[0] < waterLevel):
			swim[0] = True
			py.keyDown('space')
		elif (swim[0] == True and positionCoordinates[0] > waterLevel):
			swim[0] = False
			py.keyYp('space')
